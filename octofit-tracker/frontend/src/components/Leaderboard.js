...
function Leaderboard() {
  ...
  const columns = board.length ? Object.keys(board[0]) : [];
        ...
        <form className="row g-3 mb-4">
          <div className="col-md-8">
            ...
            <input
              ...
              placeholder="Search leaderboard rows"
            />
          </div>
        </form>
        ...
        {loading ? (
          ...
          <div className="table-responsive">
            <table className="table table-striped table-hover align-middle">
              ...
              <tbody>
                {filteredBoard.map((entry, index) => (
                  <tr key={entry.id ?? index}>
                    {columns.map((column) => (
                      <td key={column}>{renderCell(entry[column])}</td>
                    ))}
                    <td className="text-end">
                      <button className="btn btn-sm btn-primary" onClick={() => openModal(entry)}>
                        View
                      </button>
                    </td>
                  </tr>
